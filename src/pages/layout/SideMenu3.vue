...
//	import lazyLoading from '@/tools/lazy'
export default {
  data () {
    return {
...
//		}
		...
		function getComponents(items){
  			for (let i = 0; i < items.length; i++) {
		  		if(items[i].component){
		  			items[i].component = lazyLoading(items[i].component)//import('@/pages' + items[i].component + '.vue') //require('@/pages' + items[i].component + '.vue')
    ...
    pageFn ($event, child) {
      ...
      if (child.children && child.children.length > 0) {
        ...
        child.path && this.$router.push({ path: '/view/'+child.path })
      }
